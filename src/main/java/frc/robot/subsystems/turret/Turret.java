...
public class Turret extends SubsystemBase {
    ...
    private final double maxAngle = 540;
    ...
    public void launchFuel() {
        ...
        TurretIOSim.getFuelSim().launchFuel(
                ...
                Degrees.of(30),
                Degrees.of(RobotContainer.getDrive().getPose().getRotation().getDegrees() + RobotContainer.getTurret().angleToTarget()),
    ...
    public double angleToTarget() {
        ...
        if (turretRad < Math.toRadians(minAngle))
            turretRad += 2 * Math.PI;

        if (turretRad > Math.toRadians(maxAngle))
            turretRad -= 2 * Math.PI;
