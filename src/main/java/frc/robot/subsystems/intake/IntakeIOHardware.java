...
public class IntakeIOHardware implements IntakeIO {
  ...
  private SparkMax openerMotor =
      new SparkMax(
          Constants.IntakeConstants.openerMotorID,
          MotorType.kBrushless);

  private SparkMax secondOpenerMotor =
      new SparkMax(
          Constants.IntakeConstants.secondOpenerMotorID,
          MotorType.kBrushless);

  private TalonFX rollerMotor =
      new TalonFX(Constants.IntakeConstants.rollerMotorID);

  private final VelocityVoltage velocityRequest =
      new VelocityVoltage(0).withSlot(0);

  private final SparkClosedLoopController openerController;

  private double lastOpenerSetpoint = Double.NaN;

  // FIX
  private boolean lastBrakeMode = false;

  public IntakeIOHardware() {

    openerController = openerMotor.getClosedLoopController();
  ...
  public void setOpenerSetPoint(double setPoint) {
    ...
    // FIX
    if (setPoint == lastOpenerSetpoint) {
      return;
    }

    lastOpenerSetpoint = setPoint;

    openerController.setSetpoint(
        setPoint,
  ...
  public void setBrake(boolean brake) {
    ...
    openerMotor.configure(
        conf,
        ResetMode.kNoResetSafeParameters,
        PersistMode.kNoPersistParameters);

    secondOpenerMotor.configure(
        conf,
        ResetMode.kNoResetSafeParameters,
        PersistMode.kNoPersistParameters);
  }
  ...
  public void config() {
    ...
    openerConfig.encoder.positionConversionFactor(
        1.0
            / Constants.IntakeConstants.openerGearRatio
            * 360.0);
  ...
  public void updateInputs(IntakeIOInputs inputs) {
    ...
    inputs.rollerConnected =
        rollerMotor.isConnected();

    inputs.secondOpenerConnected =
        secondOpenerMotor.getLastError()
            == REVLibError.kOk;

    inputs.openerConnected =
        openerMotor.getLastError()
            == REVLibError.kOk;
    ...
    inputs.openerMotorCurrentAmps =
        openerMotor.getOutputCurrent();

    inputs.openerMotorVoltageVolts =
        openerMotor.getBusVoltage();
    ...
    inputs.isIntakeOpen =
        inputs.IntakePosition + 3
            > Constants.IntakeConstants.intakeOpenPosition;
  }
}