...
public class RobotState {
  ...
  static {
    for (int i = 1; i <= Constants.FieldConstants.aprilTagCount; i++) {
      tagPoses2d.put(
          i,
          Constants.FieldConstants.DEFAULT_APRILTAG_TYPE
              .getLayout()
              .getTagPose(i)
              .map(Pose3d::toPose2d)
              .orElse(Pose2d.kZero));
  ...
  public void periodic() {
    // Log tx/ty poses
    Pose2d[] tagPoses = new Pose2d[Constants.FieldConstants.aprilTagCount + 1];
    for (int i = 0; i < Constants.FieldConstants.aprilTagCount + 1; i++) {
                ...
                (translation) ->
                    new Pose3d(
                        new Translation3d(
                            translation.getX(),
                            translation.getY(),
                            Constants.FieldConstants.coralDiameter / 2.0),
    ...
    Leds.getInstance().closeToReef =
        poseFlipped.getTranslation().getDistance(Constants.FieldConstants.Reef.center)
                <= ledsCloseToReefDistance.get()
            && Math.abs(
                    poseFlipped
                        .getRotation()
                        .minus(
                            Constants.FieldConstants.Reef.center
